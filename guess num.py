...
def read_data_from_file():
    ...
    try:
        ...
        if len(score_list)==0:
            with open(r"C:\Users\Administrator\Desktop\test\score2.txt", "w+")as f1:
...
def write(old_list1):
    ...
    game_times =(game_times + old_list1[1])
    #print(count)
    if len(count) == 0:
        if name_flag == 0:
            min_time == 0
        ...
        count.sort()
        if count[0]<old_list1[4]:
            min_time = count[0]
        else:
            min_time = old_list1[4]        
    
    total_times = (total_times + old_list1[3])
    ava_time = int(total_times/game_times)
    if ava_time ==10:
        ava_time = 0
    write_list = [str(game_times)+'\t', str(ava_time)+'\t', str(total_times)+'\t', str(min_time)+'\t']
    #write_list1 = name+' '.join(write_list) + '\n'
    return name + "\t" +' '.join(write_list) + '\n'  #列表类型

def save_data():
    global game_times,min_time,total_times,ava_time,write_list,count,sum_ava,old_list,score_list,wanjia_list

    if name_flag == 0:
        f1 = open("score2.txt", "a+")
        old_list = ["unknowname", 0,0,0,10]
        result = write(old_list)
        #new=write(old_list)
        f1.writelines(result)
        f1.close()
    else:
        old_list = wanjia_list.split()  #都为列表
        result = write(old_list)
        #new = write(old_list)
...
def guess_number():
    global i,count,total_times,game_times,flag
    ...
    while True:
        ...
        if flag == 0:
            ...
            total_times += 10
        game_times += 1
        print("还要继续吗？  1、继续游戏切保存本轮成绩   2、不保存成绩退出  3、保存成绩并退出")
